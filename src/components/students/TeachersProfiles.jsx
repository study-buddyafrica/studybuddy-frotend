...
const TeacherProfiles = ({ userInfo, darkMode }) => {
    ...
    const fetchData = async () => {
      ...
      const fetchVideos = async () => {
        try {
          const res = await axios.get(
            `${FHOST}/lessons/api/videos/teacher/${selectedTeacher.id}`,
            { headers: authHeaders },
          );
          setVideos(
            res.status === 200 && Array.isArray(res.data?.videos)
              ? res.data.videos
              : [],
          );
        } catch {
      ...
      const fetchCourses = async () => {
        try {
          ...
          if (userInfo?.country) params.append("country", userInfo.country);

          const res = await axios.get(
            `${FHOST}/api/courses/?${params.toString()}`,
            { headers: authHeaders },
          );
          setCourses(
  ...
  const effectiveGrade =
    selectedGrade ||
    (studentGrade !== "All Grades" ? String(studentGrade) : "");
  const filteredTeachers = teachers.filter((teacher) => {
    ...
    const matchesSubject =
      ...
      (teacher.subjects || []).some(
        (subject) => String(subject) === selectedSubject,
      );
      ...
      {!selectedTeacher && (
        <div className="max-w-7xl mx-auto p-6">
          ...
          <div className="mb-6 space-y-4">
            ...
            <div>
              <div className="flex flex-wrap gap-2">
                {(selectedGrade && gradeSubjects[selectedGrade]
                  ? gradeSubjects[selectedGrade]
                  : studentGrade !== "All Grades" &&
                      gradeSubjects[String(studentGrade)]
                    ? gradeSubjects[String(studentGrade)]
                    : allSubjects
                ).map((subject) => (
                  <button
              ...
              <div className="flex flex-col md:flex-row gap-6">
                ...
                <div className="md:w-2/3">
                  ...
                  <p className="text-sm text-gray-500 mb-4">
                    Grade: {selectedTeacher.grade || "Not set"}
                  </p>
            ...
            <div className="px-6 pb-6">
              ...
              {courses.length === 0 ? (
                <p className="text-sm text-gray-500">
                  No courses available from this teacher yet.
                </p>
              ) : (
